...
public class PointOctreeNode<T> {
	...
	// Child nodes, if any
	public PointOctreeNode<T>[] children = null;
    ...
    public void GetNearest(PlacementManager placementManager, Vector3 position, float nearestSoFar, List<T> result)
    {
        //float sqrMaxDistance = nearestSoFar * nearestSoFar;
...
//#endif
        ...
        for (int i = 0; i < objects.Count; i++)
        {
            float thisMagnitude = (position - (objects[i].Obj as CelestialBody).position).magnitude;
            if (thisMagnitude <= placementManager.nearestDistanceSoFar)
            {
                //Debug.Log("nearestSoFar:" + thisMagnitude);

                placementManager.nearestDistanceSoFar = thisMagnitude;
                result.Clear();
                result.Add(objects[i].Obj);
            }
        }
        ...
        if (children != null)
        {
            for (int i = 0; i < 8; i++)
            {
                children[i].GetNearest(placementManager, position, nearestSoFar, result);
            }
        }
    }
	...
	public void DrawAllBounds(float depth) {
		...
		else
        {
            if (children != null && depth < 0)
            {
                for (int i = 0; i < 8; i++)
                {
                    children[i].DrawAllBounds(depth - 1);
                }
            }
        }
